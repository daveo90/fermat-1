...
<br>
When it comes to coding they are both the same, however the main difference is that the SuperLayers can be consumed by all 
...
will be able to consume the two previously mentioned Platforms, and so on.
...
Before we move forward with the other topics, we think it will be useful to point out the **Layout** of the system, in order for you to be able to put altogether the concepts of this document. This goes as follows:
...
Following Layers (see reference _Layout_ in the _Introduction_) we have the Sub System Classes in which you can register all the Add-on and Plug-in Developers.

-
### Developers

This last group is the one that has the logic to deliver the best Version of the Plug-in, with the possibility to perform Migrations (IE.: Update the Database of a given Plug-in, update files, etc.)
...
<br>
In the _build.gradle_ of the core we must also add all the relevant references to the _Platform's Core Modules_.
...
The _FermatPluginIdsManager_ contains all the main functionalities to manage all the Fermat _Plug-in Ids_.
<br>
